...
function asCategoryTemplate(value: unknown): CategoryTemplate {
  ...
  return {
    ...
    defaultSettings: isRecord(value.defaultSettings)
      ? value.defaultSettings
      : {},
    suggestedPlugins: toArray(value.suggestedPlugins, (item) => {
      assert(
        typeof item === "string",
        "Invalid suggestedPlugins entry in template: expected string",
      );
      return item;
    }),
    notificationEvents: toArray(value.notificationEvents, (item) => {
      assert(
        typeof item === "string",
        "Invalid notificationEvents entry in template: expected string",
      );
      return item;
    }),
  };
...
function asProjectConfig(value: unknown): ProjectConfig {
  ...
  const dbConnected =
    typeof value.dbConnected === "boolean" ? value.dbConnected : undefined;
  return {
    dbType,
...
function asCreateProjectResult(value: unknown): CreateProjectResult {
  ...
  const template: CreateProjectTemplate = {
    ...
    suggestedPlugins: toArray(value.template.suggestedPlugins, (item) => {
      assert(
        typeof item === "string",
        "Invalid suggestedPlugins entry in value.template: expected string",
      );
      return item;
    }),
    notificationEvents: toArray(value.template.notificationEvents, (item) => {
      assert(
        typeof item === "string",
        "Invalid notificationEvents entry in value.template: expected string",
      );
      return item;
    }),
  };
    ...
    integration = {
      sdkConfig: {
        baseUrl: requiredString(
          value.integration.sdkConfig.baseUrl,
          "integration.sdkConfig.baseUrl",
        ),
        projectId: requiredString(
          value.integration.sdkConfig.projectId,
          "integration.sdkConfig.projectId",
        ),
      },
      authBasePath: requiredString(
        value.integration.authBasePath,
        "integration.authBasePath",
      ),
      authBaseUrl: requiredString(
        value.integration.authBaseUrl,
        "integration.authBaseUrl",
      ),
    };
  }
...
function asProjectApiDocs(value: unknown): ProjectApiDocs {
  ...
  return {
      ...
      return {
        ...
        endpoints: toArray(module.endpoints, (endpoint): ApiDocsEndpoint => {
          ...
          return {
            ...
            requestBody: isRecord(endpoint.requestBody)
              ? endpoint.requestBody
              : undefined,
            query: isRecord(endpoint.query) ? endpoint.query : undefined,
            responseExample: isRecord(endpoint.responseExample)
    ...
    quickStart: isRecord(value.quickStart)
      ? {
          sdk:
            value.quickStart.sdk == null
              ? undefined
              : requiredString(value.quickStart.sdk, "apiDocs.quickStart.sdk"),
          curlExample:
            value.quickStart.curlExample == null
              ? undefined
              : requiredString(
                  value.quickStart.curlExample,
                  "apiDocs.quickStart.curlExample",
                ),
        }
      : undefined,
  };
...
function asUpdatedProjectResult(
  value: unknown,
): Partial<Project> & { updatedAt?: string } {
...
function asProjectTokenResult(value: unknown): {
  token: string;
  projectId: string;
  role: string;
} {
...
function asJobStatusResult(value: unknown): {
  state: string;
  progress?: number;
  result?: unknown;
  failedReason?: string;
  ...
  return {
    ...
    result: value.result,
    failedReason:
      value.failedReason == null
        ? undefined
        : requiredString(value.failedReason, "failedReason"),
  };
}
...
function asDatabaseConnectionTest(value: unknown): { connected: boolean } {
  assert(isRecord(value), "Invalid database connection test response");
  return {
    connected: requiredBoolean(value.connected, "connected"),
  };
}
...
const ProjectService = {
  ...
  ): Promise<Partial<Project> & { updatedAt?: string }> {
    assertProjectId(projectId);
  ...
  async testDbConnection(projectId: string): Promise<{ connected: boolean }> {
    assertProjectId(projectId);
    const { data } = await api.post(
      `/projects/${projectId}/config/test-db-connection`,
    );
    return asDatabaseConnectionTest(unwrapDataEnvelope(data));
  },
  ...
  async updateCors(
    ...
    assertProjectId(projectId);
    const { data } = await api.patch(`/projects/${projectId}/cors`, {
      allowedOrigins,
    });
  ...
  async getProjectToken(
    projectId: string,
  ): Promise<{ token: string; projectId: string; role: string }> {
    assertProjectId(projectId);
  ...
  async migrateUsers(
    projectId: string,
    payload: MigrateUsersPayload,
  ): Promise<{ jobId: string }> {
    assertProjectId(projectId);
    assertNonEmptyString(payload.selector, "selector");
    const { data } = await api.post(
      `/projects/${projectId}/migrate-users`,
      payload,
    );
    return asJobIdResult(unwrapDataEnvelope(data));
  },

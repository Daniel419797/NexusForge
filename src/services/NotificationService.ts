...
function asNotification(value: unknown): Notification {
  ...
  return {
    ...
    body:
      value.body == null
        ? null
        : requiredString(value.body, "notification.body"),
...
const NotificationService = {
  ...
  async markAsRead(id: string, projectId?: string): Promise<Notification> {
  ...
  async markAllAsRead(
    ...
    const { data } = await api.post("/notifications/read-all", undefined, {
      headers: projectId ? { "x-project-id": projectId } : undefined,
    });
  ...
  async getDevices(projectId?: string): Promise<PushDevice[]> {
  ...
  async removeDevice(deviceId: string, projectId?: string): Promise<void> {

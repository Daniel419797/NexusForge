...
export default function OAuthButtons({ mode }: OAuthButtonsProps) {
    ...
    // requests (?_rsc=...) which fail for backend redirect endpoints.
    ...
    return (
        <div className="space-y-3">
            <button
                type="button"
                onClick={() => handleOAuth(googleUrl)}
                className="flex items-center justify-center gap-3 w-full px-4 py-2.5 rounded-lg border border-border bg-card hover:bg-secondary/50 transition-all duration-200 text-sm font-medium"
            >

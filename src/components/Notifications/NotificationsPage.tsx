...
export default function NotificationsPage() {
  ...
  const updatePref = (key: keyof NotificationPreferencesValue) => {
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      await NotificationService.markAsRead(id, activeProjectId);
  ...
  const handleMarkAllRead = async () => {
    try {
      await NotificationService.markAllAsRead(activeProjectId);
  ...
  const fetchDevices = async () => {
    ...
    try {
      const data = await NotificationService.getDevices(activeProjectId);
      setDevices(data);
    } catch {
      /* ignore */
    } finally {
  ...
  const handleRemoveDevice = async (deviceId: string) => {
    ...
    try {
      await NotificationService.removeDevice(deviceId, activeProjectId);
      ...
      >
        <div>
          ...
          <p className="text-sm text-white/50 mt-2">
            Stay updated on your platform activity.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <ElectricRippleButton
            ...
            onClick={() => {
              ...
              if (!showDevices && devices.length === 0) fetchDevices();
            }}
          >
          ...
          >
            ...
            {devicesLoading ? (
              ...
              <p className="text-sm text-white/50 text-center py-4">
                No push devices registered. Enable push in your browser or app.
              </p>
            ) : (
              devices.map((device) => (
                <div
      ...
      <div className="space-y-3">
        {loading &&
          [1, 2, 3].map((i) => (
            <div
              key={i}
              className="w-full h-24 rounded-2xl bg-white/[0.03] animate-pulse"
            />
          ))}
          ...
          {!loading &&
            notifications.map((notif, index) => (
              <ScrollReveal key={notif.id} direction="up" delay={index * 0.04}>
                <div
                  className={`rounded-xl border px-4 py-3 ${!notif.read ? "border-rose-500/20 bg-white/[0.02] ring-1 ring-rose-500/20" : "border-white/[0.06] bg-white/[0.02]"}`}
                >
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-2 mb-1">
                        <h3
                          className={`text-sm font-semibold ${!notif.read ? "text-white" : "text-white/60"}`}
                        >
                          {notif.title}
                        </h3>
